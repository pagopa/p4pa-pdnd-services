...
//region test signJwtRSA
  ...
  void whenSignJwtRSAThenOk() throws InvalidKeySpecException, NoSuchAlgorithmException, IOException {
    // Given
    RSASSASigner signer = new RSASSASigner(CertUtils.pemKey2PrivateKey(PRIVATE_KEY));
    JWTVerifier verifier = JWT.require(Algorithm.RSA512(CertUtils.pemPub2PublicKey(PUBLIC_KEY))).build();
